class _Templates extends EventTarget {
    ...
    // 
    setLinkedImportId(element, id) {
        element.setAttribute(this.ATTRIBUTES.LINKED_TEMPLATE_IMPORT_ID, `${id}`);
    }
    getLinkedImportId(element) {
        return element.getAttribute(this.ATTRIBUTES.LINKED_TEMPLATE_IMPORT_ID);
    }
    removeAllImportGarbagesById(id) {
        document.querySelectorAll(`[${this.ATTRIBUTES.LINKED_TEMPLATE_IMPORT_ID}="${id}"]`).forEach(garbage_element => { garbage_element.remove(); });
    }
    // 
    async loadImport(template_import, depth = 0) {
        const import_id = Date.now().toString(16) + "." + depth;
        // 
        if (!this.isTemplateImport(template_import))
            throw new Error("Unable to load template : invalid element passed.");
        if (!template_import.hasAttribute("src"))
            throw new Error("Unable to load template : missing path.");
        // 
        const template_source = template_import.getAttribute("src");
        ...
        if (fetch_template_response.status == 404)
            throw new Error("Unable to load template : template not found.");
        else if (fetch_template_response.status != 200)
            throw new Error("Unable to load template : template not fetched.");
        // 
        ...
        let template = template_import.querySelector(":scope > template");
        if (!template)
            throw new Error("Unable to load template : HTMLTemplateElement not found.");
        template.remove();
        this.setLinkedImportId(template, import_id);
        const template_tokens = Array.from(template_import.querySelectorAll(":scope > " + this.ELEMENTS.TEMPLATE_TOKEN));
        for (let i = 0; i < template_tokens.length; ++i) {
            ...
            for (let j = 0; j < template_tokens_to_replace.length; ++j) {
                await (async () => {
                    ...
                    if (!(template_token_to_replace instanceof HTMLElement))
                        return;
                    if (this.isTemplateImport(template_token_to_replace.parentElement))
                        return;
                    await mapChildNodesCopy(template_token, async (childnode) => {
                        if (!(childnode instanceof Node))
                            return;
                        if (childnode instanceof HTMLElement)
                            this.setLinkedImportId(childnode, import_id);
    ...
    //
    async loadImports(container = document, depth = 0) {
        if (this.isTemplateImport(container))
            await this.loadImport(container, depth);
        else {
...
async function renewScriptElement(script_element) {
...
async function renewContainerScripts(container) {
    if (container instanceof HTMLScriptElement)
        await renewScriptElement(container);
    else {
        const childnodes = Array.from(container.childNodes);
        for (let i = 0; i < childnodes.length; ++i) {
            let childnode = childnodes[i];
